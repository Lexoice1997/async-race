...
export interface CarProps {
  ...
  editCar: (name: string, color: string, id: number) => void;
  drive: boolean;
  getWinnerCar: (bool: boolean,id: number, name: string, duration: number) => void;
  isWinner?: boolean;
}
...
const Car: React.FC<CarProps> = ({id, name, color, removeCar, editCar, drive, getWinnerCar, isWinner}) => {
    ...
    axios.patch(`http://127.0.0.1:3000/engine?id=${id}&status=started`)
      ...
      .then(() => {
        axios.patch(`http://127.0.0.1:3000/engine?id=${id}&status=drive`)
          ...
          .finally(() => {
            setSecondBtn(false);
            axios.patch(`http://127.0.0.1:3000/engine?id=${id}&status=stopped`)
          });
      })
  };

  const animate = ({timing, draw, duration}: Animate) => {
    ...
    requestAnimationFrame(function animate(time) {
      ...
      if (timeFraction < 1) {
        ...
        if (isWinner) {
          getWinnerCar(false, id, name, duration);
        }
      }
    });
  };
  ...
  return (
    <div className="car">
      <div className="car-settings">
        <button onClick={() => editCar(name, color, id)}>SELECT</button>
        <button onClick={() => removeCar(id)}>REMOVE</button>
        <span>{name}</span>
      </div>
      <div className="car-helm">
        <button onClick={() => startDrive()} disabled={firstBtn}>A</button>
        <button onClick={() => stopDrive()} disabled={secondBtn}>B</button>

        <div className="car-svg">
          <svg version="1.0" xmlns="http://www.w3.org/2000/svg" ref={carRef}
