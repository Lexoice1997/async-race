...
const Winners = () => {
  ...
  const [carsWin, setCarsWin] = useState([]);
  const [carsCount, setCarsCount] = useState<number>(0);
  const [totalPages, setTotalPages] = useState(0);
  const [limit] = useState(7);
  const [page, setPage] = useState(1);
  const [sortFlag, setSortFlag] = useState({id: true, wins: true, time: true});
  ...
  async function fetchCars() {
    const responseWinners = await CarService.getAllWinners(limit, page);
    const totalCount: number = +responseWinners.headers['x-total-count'];
    const responseCars = await axios.get('http://127.0.0.1:3000/garage');

    setCars(responseCars.data);
    setCarsCount(totalCount);
    setCarsWin(responseWinners.data);
    setTotalPages(getPageCount(totalCount, limit));
  }

  const getCar = (id: number): {name: string, color: string} => {
    const car = cars.filter((item: IWinners) => item.id === id);
    return car[0];
  };

  const sortById = async () => {
    if (sortFlag.id) {
      setSortFlag({...sortFlag, id: false});
      const response = await axios.get(`http://127.0.0.1:3000/winners?_sort=id&_order=ASC`);
      setCarsWin(response.data)
    } else {
      setSortFlag({...sortFlag, id: true});
      const response = await axios.get(`http://127.0.0.1:3000/winners?_sort=id&_order=DESC`);
      setCarsWin(response.data)
    }
  };
  ...
  const sortByWins = async () => {
    if (sortFlag.wins) {
      setSortFlag({...sortFlag, wins: false});
      const response = await axios.get(`http://127.0.0.1:3000/winners?_sort=wins&_order=ASC`);
      setCarsWin(response.data)
    } else {
      setSortFlag({...sortFlag, wins: true});
      const response = await axios.get(`http://127.0.0.1:3000/winners?_sort=wins&_order=DESC`);
      setCarsWin(response.data)
    }
  };
  ...
  const sortByTime = async () => {
    if (sortFlag.time) {
      setSortFlag({...sortFlag, time: false});
      const response = await axios.get(`http://127.0.0.1:3000/winners?_sort=time&_order=ASC`);
      setCarsWin(response.data)
    } else {
      setSortFlag({...sortFlag, time: true});
      const response = await axios.get(`http://127.0.0.1:3000/winners?_sort=time&_order=DESC`);
      setCarsWin(response.data)
    }
  };
  ...
  return (
    <div>
      ...
      <ul className="car_list-head">
        <li onClick={sortById} className="clicks">Number</li>
        <li>Car</li>
        <li>Name</li>
        <li onClick={sortByWins}  className="clicks">Wins</li>
        <li onClick={sortByTime}  className="clicks">Best Time</li>
      </ul>

      {carsWin.map((item: IWinners, index) => (
        <ul className="car_list car_list-head" key={item.id}>
          <li>{index + 1}</li>
          <li>
            <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                 width="50.000000pt" height="50.000000pt" viewBox="0 0 1280.000000 640.000000"
                 preserveAspectRatio="xMidYMid meet">
              <g transform="translate(0.000000,640.000000) scale(0.100000,-0.100000)"
                 fill={getCar(item.id)?.color} stroke="none">
